...
int main()
{
    ...
    // creating hash table
    int hash_table[number_of_elements];
    ...
    while(dummy == 1)
    {
        printf("Enter 1 to insert an element in the hash table\n");
        ...
        if(response == 1)
        {
            printf("Enter the data to be inserted in the hash table \n");
            int data;
            scanf("%d",&data);
            fflush(stdin);
            int ans = insert_data_in_hash_table(data,number_of_elements,hash_table);
...
int insert_data_in_hash_table(int data,int number_of_elements, int * ptr_to_hash_table)
{
    ...
    {
        int ans = handle_collision_and_display_hash_table(data, number_of_elements, ptr_to_hash_table);
        return ans;
...
int handle_collision_and_display_hash_table(int data , int number_of_elements , int * ptr_to_hash_table)
{
    ...
    int ans = is_hash_table_full(ptr_to_hash_table,number_of_elements);
    ...
    search_empty_place_after_collision_and_insert(data,number_of_elements,ptr_to_hash_table,index);
    display_hash_table(number_of_elements,ptr_to_hash_table);
    return 1;
