...
int *ptr_to_bos2;
int *ptr_to_tos_variable2;
int MAX2=100;
int stack2[100];
int top2;

void push_into_stack2(int);
int pop_from_stack2();
...
void create_stack2()
{
    ...
    ptr_to_bos2 = &(stack2[0]);
    top2 = -1;
    ptr_to_tos_variable2 = &top2;

}



void push_into_stack2(int element)
{
    if(*(ptr_to_tos_variable2) == MAX2 - 1)
    {
        printf("The stack2 is full, cannot perform push operation\n");
        return;
    }

    *(ptr_to_tos_variable2) +=1;
    *(ptr_to_bos2 + *(ptr_to_tos_variable2) ) = element;
...
int pop_from_stack2()
{
    if(*(ptr_to_tos_variable2) == -1)
    {
        printf("The stack2 is empty, pop operation cannot be performed\n");
        printf("Please push more items and then try to pop them later\n");
        return 0;
    }

    int element_to_be_popped = ptr_to_bos2[*(ptr_to_tos_variable2)];

    printf("Popping %d from the stack2\n",element_to_be_popped);
    ptr_to_bos2[*(ptr_to_tos_variable2)] = 0;
    *(ptr_to_tos_variable2) -= 1;
    printf("Successfully popped out %d from stack2\n",element_to_be_popped);
    return element_to_be_popped;
}

void display_tos_to_bos2()
{
    int i;
    printf("Displaying stack2 from top1 to bottom : \n");
    for(i=*(ptr_to_tos_variable2); i >= 0; i--)
    {
        printf("%d\n", ptr_to_bos2[i]);
    }
}
